...
public class TopHeadlinesDataSource extends PageKeyedDataSource<Integer, Response.Article> {
    ...
    public TopHeadlinesDataSource(String category) {
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull LoadInitialCallback<Integer, Response.Article> callback) {
        ...
        ApiClient.getInstance().getHeadlinesApi()
                .getHeadlines(COUNTRY, category, PAGE_SIZE, FIRST_PAGE, API_KEY)
    ...
    public void loadBefore(@NonNull LoadParams<Integer> params, @NonNull LoadCallback<Integer, Response.Article> callback) {
        ...
        ApiClient.getInstance().getHeadlinesApi()
                .getHeadlines(COUNTRY, category, PAGE_SIZE, params.key, API_KEY)
                .enqueue(new Callback<Response>() {
                    @Override
                    public void onResponse(Call<Response> call, retrofit2.Response<Response> response) {

                        if (response.body() != null) {
                            Integer adjacentPageKey = (params.key > FIRST_PAGE) ? params.key - 1 : null;
